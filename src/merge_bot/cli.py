...
def parse_cli_arguments(testing_args=None):
    ...
    has_keys = bool(args.github_app_key or args.github_cloner_key)

    if has_token and has_keys:
        parser.error(
            "--github-token and (--github-app-key/--github-cloner-key) "
            "are mutually exclusive."
        )

    if not has_token and not (args.github_app_key and args.github_cloner_key):
        parser.error(
            "Either --github-token or both --github-app-key and "
            "--github-cloner-key must be provided."
        )
